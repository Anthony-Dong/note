...
```pro
# 开启一级缓存   statement/session   所以默认就是开启了一级缓存 
mybatis.configuration.local-cache-scope=session
```

但是对于spring,`SqlSession`对象确实只有一个,但是他只是一个接口,需要实现类,所以每一次请求获取的`SqlSession` 对象的默认实现类 `DefaultSqlSession` 是不一样的,所以,并没有实现缓存机制,这是我两次请求的日志信息,所以其实一级缓存无效的

```java
JDBC Connection [HikariProxyConnection@1202527432 wrapping com.mysql.jdbc.JDBC4Connection@a68ba22] will not be managed by Spring
==>  Preparing: select DEPTNO, DNAME, LOC from dept where DEPTNO = ? 
==> Parameters: 10(Integer)
<==    Columns: DEPTNO, DNAME, LOC
<==        Row: 10, ACCOUNTING, NEW YOURK
<==      Total: 1
Closing non transactional SqlSession [org.apache.ibatis.session.defaults.DefaultSqlSession@c561d89]
Creating a new SqlSession
SqlSession [org.apache.ibatis.session.defaults.DefaultSqlSession@63c62bd5] was not registered for synchronization because synchronization is not active
JDBC Connection [HikariProxyConnection@1828837853 wrapping com.mysql.jdbc.JDBC4Connection@a68ba22] will not be managed by Spring
==>  Preparing: select DEPTNO, DNAME, LOC from dept where DEPTNO = ? 
==> Parameters: 10(Integer)
<==    Columns: DEPTNO, DNAME, LOC
<==        Row: 10, ACCOUNTING, NEW YOURK
...
```java
Closing non transactional SqlSession [org.apache.ibatis.session.defaults.DefaultSqlSession@5825996c]
Creating a new SqlSession
SqlSession [org.apache.ibatis.session.defaults.DefaultSqlSession@14d81ba5] was not registered for synchronization because synchronization is not active
...
public class MyMybatisCache implements Cache {
    ...
    private static Logger LOGGER = LogManager.getLogger(MyMybatisCache.class);
    ...
    public MyMybatisCache(final String id) {
    ...
    public void putObject(Object key, Object value) {
        ...
        LOGGER.info("Object key : {}", key.toString());

        LOGGER.info("Object value : {}", value.toString());
